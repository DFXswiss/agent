...
Optional repository scope is configured separately in `.github/pr-guard.json` (`pr-guard/v1`): explicit `enforce` / `exclude` target-branch lists and an `enforce`|`exclude` default. The Agent reads that file from an immutable commit of the repository default branch (metadata from the PR base repository only) and evaluates exact case-sensitive branch names; missing configuration retains legacy enforce-all except exact target `main`, and malformed or denied configuration fails closed. Exact target `main` is always out of scope (built-in; nothing to check), even when the file is missing or `a38.enforce` lists `main`. The default branch is only the trusted location of that file, not an implicit enforce. Excluded targets are `not_applicable` without loading reports or policy. See [dfx pr guard](a38-guard.md) for the full contract, including freshness checks and migration rules for config edits.
...
| `.github/pr-guard.json` | `pr-guard/v1` target-branch scope. Typical private integration branch: `enforce` that branch, built-in exact `main` stays out of scope. |
...
6. Add `.github/pr-guard.json` from [the example](../examples/pr-guard.json) with this repository's actual target-branch names.
...
Worked public example of `jobs[]` / `exclusions[]` shape: [examples/a38.json](../examples/a38.json). A complete private adopter (eight required jobs, adapter `commands` / `compose` / `http-smoke` / `immutable`, no workflow interpreter) is DFXswiss/backend `.github/a38.json` on its default branch.
