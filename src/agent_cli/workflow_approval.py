...
def approve_workflow_runs(api: Any, assessment: Any, *, dry_run: bool = False) -> list[dict[str, Any]]:
    ...
    for stale in _stale_held(inventory, pull, candidates):
        ...
        if not dry_run:
            ...
            if status == 202:
                cancel_comment = record_workflow_cancel(
                    api, assessment, run, create=cancel_comment is None, existing=cancel_comment
                )
        result.append(
            {
                "run_id": stale["id"],
    ...
    for path, candidate in sorted(candidates.items()):
        ...
        from .readme_only import pull_is_guard_docs_only, pull_is_markdown_only
        docs_waiver = _docs_report_waiver(
            getattr(assessment, "write_ready_reason", "")
        ) and (
            pull_is_markdown_only(api, final.repo, final.number)
            or pull_is_guard_docs_only(api, final.repo, final.number)
        )
        ...
        if not dry_run:
            ...
            auth_comment = record_workflow_approval(
                api, assessment, run, create=auth_comment is None, existing=auth_comment
            )
