...
Authors who currently have `write`, `maintain`, or `admin` on the target repository are not required to post that author report for Ready; the guard waives the report gate with an explicit status description. No-write authors (including forks and triage-only collaborators) must still publish a valid author report before Ready. Policy, workflow inventory, and migration failures are never waived by write access.
...
The **author local-CI report** is required for Ready when the PR author lacks `write`, `maintain`, or `admin` on the **target** repository. If the author currently has one of those roles, the report is not required; enforce status succeeds with an explicit waiver description. The same waiver applies when a human collaborator with `write`/`maintain`/`admin` is the latest `ready_for_review` timeline actor (maintainer Ready on a no-write author's PR). Bots and apps cannot grant the waiver. `MEMBER` / `author_association` is not write. A 404 or denied collaborator-permission lookup does not grant it. Invalid policy, unclassified workflows, and pr-guard migration failures are **not** waived. No-write authors (forks, triage-only, outside collaborators) who mark Ready without a valid author report still fail; lifecycle still auto-drafts them. Lifecycle must not convert a Ready PR back to Draft while that write hold applies.
