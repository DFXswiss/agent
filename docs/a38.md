...
The PR author processes every required local job and posts the generated report as a PR comment. Jobs listed in optional `readme_only.omit_jobs` are not executed when the change set is independently README-only; they are recorded as `result=not_applicable`. The dfx pr guard explains those rules and verifies the report against the repository's trusted policy.
...
Authors who are GitHub `User`s and currently have `write`, `maintain`, or `admin` on the target repository are not required to post that author report for Ready; the same waiver applies when a `User` with one of those roles is the latest `ready_for_review` actor. The guard waives only the report gate, with an explicit status description. Bots and apps cannot grant it. No-write authors (including forks and triage-only collaborators) must still publish a valid author report before Ready. Policy, workflow inventory, and migration failures are never waived by write access.
