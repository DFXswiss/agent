...
- Optional Telegram status posts (`TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID`) are a script HTTP side-effect. They are not person pings and not the hub. Missing env is a no-op. **Working** is a Ja/Nein read of the Grok TUI in the session's tmux pane (`Thinking…`, `Preparing …`, `Waiting for response`, `[stop]`, `Esc:cancel`). Pane flicker, process lists, and log mtimes are not that question. `not working` is posted only after the probe stays Nein for 10 minutes (`TELEGRAM_IDLE_SECONDS`, default 600). A brief gap between turns does not page. A send failure does not stop the loop.
