...
- Lifecycle: only `agent init` runs `initdb`. Commands that open the store start an existing stopped cluster and otherwise exit with `run agent init`; they never create one. `agent pg status` reports it without starting it; `agent pg stop` and `agent daemon --uninstall` stop it. `AGENT_PG_DSN` bypasses all of this.
