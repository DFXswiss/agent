...
- The follow loop does not type into tmux while the TUI shows an in-flight turn. After 2 minutes of a truly idle pane with a pending assignment it sends one locked continue line (`Setze die Arbeit fort.`), not a knock and not a closed question. Closed questions remain available to `tick(..., ask=True)` for tests. A Grok tool-approval modal (`1/3:select` → Enter) is still cleared. The footer badge `always-approve` is not a working signal.
