...
- The follow loop does not type into tmux except to confirm a Grok tool-approval modal (`1/3:select` → Enter). It does not knock after the session exists, and it does not ask closed questions while following. Closed questions remain available to `tick(..., ask=True)` for tests. A Grok tool-approval modal is the only in-session keystroke. The footer badge `always-approve` is not a working signal. When `ask=True`, idle waits `QUIET_SECONDS` then asks only: done? (`Ja` / `Nein`); if no, can you finish alone vs a blocking problem.
...
- Optional Telegram status posts (`TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID`) are a script HTTP side-effect. They are not person pings and not the hub. Missing env is a no-op. **Working** is a Ja/Nein read of the Grok TUI in the session's tmux pane (`Thinking…`, `Preparing …`, `Waiting for response`, `[stop]`, `Esc:cancel`). Pane flicker, process lists, and log mtimes are not that question. Each follow tick captures the pane once and uses that snapshot for both the loop and Telegram. `not working` is posted only after that probe stays Nein for 10 minutes (`TELEGRAM_IDLE_SECONDS`, default 600). Follow start resets the idle clock so a stale timestamp cannot page. A send failure does not stop the loop.
