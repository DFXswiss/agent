...
- The follow loop does not type into tmux while the TUI shows an in-flight turn. A single idle capture is not enough: idle must be consecutive. After 3 idle ticks (~3 min at the 60s poll) it sends one locked continue line (`Setze die Arbeit fort.`). It does not knock and does not ask closed questions. Closed questions remain available to `tick(..., ask=True)` for tests. A Grok tool-approval modal (`1/3:select` → Enter) is still cleared. The footer badge `always-approve` is not a working signal.
