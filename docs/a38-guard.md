...
On an open **draft** in `enforce` mode the guard still publishes or updates its educational comment — a friendly greeting plus a link to the A38 quality rules (`standard_url`) only; no draft-status lecture, no missing-report or tool-attribution lecture, no write-waiver or markdown-only lecture, and no details block (jobs, problems, run command). Current problems and run instructions appear in the comment once the pull request is Ready (`draft=false`). The guard does **not** create or update a blocking `A38 / report (<target>)` commit status on draft (and does not post an invalidating `error` status on draft). Configured `not_applicable` exclusions may still write success on that context only to clear a wrong prior status; that is not a test-pass claim. Process exit is 0 so `dfx pr guard` is not red merely because a draft lacks an author report. Ready (`draft=false`) publishes success for valid author-report evidence or for a report waiver (write collaborator, or independently confirmed markdown-only); missing or invalid evidence without a waiver is failure. Markdown-only does not hold Ready through red CI. Independently, the guard scans the live PR title, PR body, and every PR commit message plus author/committer identity (not the diff, not comments). A `generated-with` banner, AI `Co-Authored-By` trailer, AI session header, or AI author identity fails the assessment with `hard_fail`; `dfx pr guard` then exits 1 even on a draft and even in observe mode, and authors must remove those trailers. The draft exemption for the blocking `A38 / report (<target>)` commit status remains: that status is still omitted on draft (except the existing `not_applicable` success-clear carve-out); the Actions job exit code is the red CI for tool-attribution.
